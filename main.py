"""
╔══════════════════════════════════════════════════════════╗
║         CYBERLOAD — Descargador Cyberpunk  v1.5          ║
...
Cambios v1.5:
    · Crash log renombrado a cyberload_error.txt
      (rutas candidatas: /sdcard/, /storage/emulated/0/, CWD)
    · Solicitud de permisos dinámica en on_start()
      (INTERNET, READ/WRITE_EXTERNAL_STORAGE, MANAGE_EXTERNAL_STORAGE)
    · try-except global en __main__ con escritura de cyberload_error.txt
    · on_start() separado de build() para compatibilidad con Android
...
#
#  Intenta tres rutas en orden; escribe en la primera que
#  funcione. El nombre del archivo es cyberload_error.txt
#  en todas las rutas para facilitar la búsqueda en /sdcard/.
# ─────────────────────────────────────────────────────────
...
def _escribir_crash_log(exc: Exception) -> None:
    ...
    # /sdcard/ es la más fácil de acceder desde el explorador de archivos.
    rutas_candidatas = [
...
class CyberLoadApp(MDApp):
    ...
    Estructura del ciclo de vida:
        build()    → construye la UI (sin permisos aún)
        on_start() → solicita permisos dinámicos de Android
    """
    ...
    #
    #  CRÍTICO: Los permisos DEBEN pedirse en on_start() y NO
    #  en build(). En build() la actividad Android aún no está
    #  lista para mostrar diálogos de permiso, lo que causa
    #  crash inmediato en Android 6+ (API 23+).
    #
    #  El bloque try/except ImportError permite que el archivo
    #  funcione también en Pydroid 3 y PC donde el módulo
    #  'android' no existe.
    # ─────────────────────────────────────────────────────
    ...
    def on_start(self):
        ...
        try:
            ...
            # Se añade de forma condicional para no crashear en Android 8/9.
            try:
                permisos_requeridos.append(Permission.MANAGE_EXTERNAL_STORAGE)
            except AttributeError:
                pass  # No disponible en esta versión de python-for-android
            ...
            # Estamos en PC o Pydroid 3 — no se necesitan permisos
            self.ui._set_estado(
                "[ Esperando link... ]",
                color=(0, 0.9, 1, 0.85),
            )
...
#  la app se cierre, facilitando el diagnóstico sin logcat.
#
#  Rutas candidatas para el archivo de error (en orden):
#    1. /sdcard/cyberload_error.txt          ← más fácil de encontrar
#    2. /storage/emulated/0/cyberload_error.txt
#    3. <directorio de main.py>/cyberload_error.txt
# ─────────────────────────────────────────────────────────
