"""
╔══════════════════════════════════════════════════════════╗
║         CYBERLOAD — Descargador Cyberpunk  v1.4          ║
...
Cambios v1.4:
    ...
    · try-except global en __main__ con escritura de crash_log.txt
...
#  Intenta tres rutas en orden; escribe en la primera que funcione.
# ─────────────────────────────────────────────────────────

def _escribir_crash_log(exc: Exception) -> None:
    """
    Guarda el traceback completo en crash_log.txt.
    Prueba /sdcard/, /storage/emulated/0/ y el CWD.
    """
    contenido = (
        f"=== CYBERLOAD CRASH LOG ===\n"
    ...
    rutas_candidatas = [
        "/sdcard/cyberload_crash.txt",
        "/storage/emulated/0/cyberload_crash.txt",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "cyberload_crash.txt"),
    ]
    ...
    for ruta in rutas_candidatas:
        try:
            with open(ruta, "w", encoding="utf-8") as f:
                f.write(contenido)
            print(f"[CRASH LOG] Guardado en: {ruta}")
            return
        except Exception:
            continue

    # Último recurso: imprimir en logcat
    print(contenido)
...
#  queda registrada en cyberload_crash.txt antes de que la
#  app se cierre, facilitando el diagnóstico sin logcat.
# ─────────────────────────────────────────────────────────
