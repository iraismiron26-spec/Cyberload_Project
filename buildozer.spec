# ╔══════════════════════════════════════════════════════════════════╗
# ║  buildozer.spec — CYBERLOAD MSTL v1.6                          ║
# ║  Generado para: KivyMD + yt-dlp + Instaloader + Pillow         ║
# ║  Target: Android 8+ (API 26+) · Sin FFmpeg · APK lista         ║
# ║                                                                  ║
# ║  Cambios v1.6:                                                  ║
# ║  · p4a.branch = master  (sincronizado con buildozer master)     ║
# ║  · android.accept_sdk_license = True  (verificado activo)       ║
# ║  · version bump 1.5 → 1.6  (fuerza reinstalación limpia)       ║
...
#  · charset-normalizer → detección de encoding. Dependencia interna de requests.
...
# ── Log ───────────────────────────────────────────────────────────
# 2 = verbose (recomendado para depurar en GitHub Actions)
log_level = 2
...
# ── Accept SDK License ────────────────────────────────────────────
# CRÍTICO: Sin esta línea activa (sin #), buildozer puede detenerse
# en medio de la compilación esperando confirmación de licencias.
...
# ── p4a branch — CAMBIO CRÍTICO v1.6 ─────────────────────────────
# p4a.branch = master sincroniza python-for-android con la versión
# master de Buildozer que usa el workflow v8.
#
# Por qué es necesario:
#   · buildozer_version: master en el workflow usa APIs y recetas
#     que solo existen en la rama master de p4a.
#   · Usar la rama estable de p4a con buildozer master puede causar
#     incompatibilidades en las recetas de openssl, kivy y kivymd.
#   · La rama master de p4a tiene soporte actualizado para NDK 25b
#     y las arquitecturas arm64-v8a + armeabi-v7a simultáneas.
# ──────────────────────────────────────────────────────────────────
p4a.branch = master
